...
import { RETURN_STATUS_LABEL, DISPOSITION_ICON } from '../../constants';
import type { SalesReturn, MainStackParamList } from '../../types';
...
export default function SalesReturnsScreen() {
  ...
  const { filter, setFilter, search, setSearch, returns, loading, refreshing, error, filters, refresh } = useSalesReturns();

  function ReturnRow({ item }: { item: SalesReturn }) {
    ...
    return (
      <TouchableOpacity
        onPress={() => nav.navigate('SalesReturnDetail', { returnId: item.id })}
        style={{
      ...
      >
        ...
        <Text style={{ color: colors.textPrimary, fontSize: fontSize.xs }}>{item.customer_name}</Text>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ backgroundColor: colors.surface, borderBottomColor: colors.border, borderBottomWidth: 1, padding: spacing.base }}>
        <Text style={{ color: colors.textPrimary, fontSize: fontSize.lg, fontWeight: fontWeight.bold }}>Sales Returns</Text>
      </View>

      <View style={{ padding: spacing.base, backgroundColor: colors.surface }}>
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search by return# or customer..." />
      </View>
      ...
      {loading ? <Loader fullScreen /> : error ? <ErrorState message={error} onRetry={refresh} /> : (
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="🔄" title="No returns" description="No sales returns found." />}
        />
      )}
    </SafeAreaView>
  );
